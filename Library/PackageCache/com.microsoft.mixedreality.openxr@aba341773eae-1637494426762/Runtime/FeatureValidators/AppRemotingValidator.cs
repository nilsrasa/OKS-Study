        ...
        internal static void GetValidationChecks(OpenXRFeature feature, List<ValidationRule> results, BuildTargetGroup targetGroup)
        {
            ...
            results.Add(new ValidationRule(feature)
            {
                ...
                checkPredicate = () =>
                {
                    ...
                    Remoting.PlayModeRemotingPlugin playModeRemotingFeature = OpenXRSettings.Instance.GetFeature<Remoting.PlayModeRemotingPlugin>();;
                    return !playModeRemotingFeature.enabled;
                },
                fixIt = () =>
                {
                    var buildTarget = BuildTargetGroup.Standalone;
                    var playModeRemotingFeatureSetId = "com.microsoft.openxr.featureset.playmoderemoting";
                    Remoting.PlayModeRemotingPlugin playModeRemotingFeature = OpenXRSettings.Instance.GetFeature<PlayModeRemotingPlugin>();
                    var featureSet = OpenXRFeatureSetManager.GetFeatureSetWithId(buildTarget, playModeRemotingFeatureSetId);

                    featureSet.isEnabled = false;
                    OpenXREditorSettingsInstanceSetFeatureSetSelected(buildTarget, playModeRemotingFeatureSetId, false);
